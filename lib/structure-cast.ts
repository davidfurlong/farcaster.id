...
export const channelRegexForSplit =
  /(^|\s|\.)(\/[a-z0-9][a-z0-9-]{0,16}(?:\.eth)?)/gi;
export const channelRegex = /(?:^|\s)\/([a-z0-9][a-z0-9-]{0,16}(?:\.eth)?)/gi;
...
export type StructuredCastChannel = {
  type: "channel";
  serializedContent: string;
};
...
function extractChannels(
  ...
  return structuredCast.serializedContent
    .split(channelRegexForSplit)
    .map((part, i) => {
      const e = channelRegex.exec(part);
      if (!e) {
        return { type: "plaintext", serializedContent: part };
      }
      return { type: "channel", serializedContent: part };
    });
}
...
export function convertCastPlainTextToStructured({
  ...
  structuredCast = structuredCast.flatMap((x) => extractChannels(x));
